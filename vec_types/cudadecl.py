...
#all libdevice functions are redefined as CallableTemplates so that external types can define new lowering behaviour
...
def register_op(func, factory, *args):
    ...
    #     func = func.__name__
    api[func] = types.Function(factory(func, *args))
...
class CudafuncsModuleTemplate(AttributeTemplate):
    key = types.Module(cudafuncs)
    ...
    def __getattr__(self, attr):
        ...
        if attr[8:] in api:
            ...
            raise AttributeError(f"module 'cudafuncs' has no attribute '{attr[8:]}'")
